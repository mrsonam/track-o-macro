      ...
      <div className="relative">
        <div className="mb-5 flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-sky-500/15 text-sky-700 ring-1 ring-sky-500/20">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                {isToday ? (
                    <span className="rounded-md bg-sky-500/15 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-sky-700">
                    Today
                  </span>
                ) : (
                  <span className="rounded-md bg-white/60 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-zinc-500">
                    Past day
                  </span>
                )}
              </div>
              <p className="text-xs font-semibold text-zinc-600">{heading}</p>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          <div className="space-y-4" aria-busy="true" aria-label="Loading hydration">
            <div className="h-9 w-40 rounded-lg bg-white/60" />
            <div className="h-2.5 w-full rounded-full bg-white/60" />
            <div className="flex flex-wrap gap-2">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="h-9 w-16 rounded-xl bg-white/60" />
              ))}
            </div>
          ...
          <>
            ...
            >
              <div className="mb-3 flex items-end justify-between gap-3">
                <div>
                  ...
                  <p
                    className="font-mono text-3xl font-black tabular-nums tracking-tight text-[#171412]"
                    aria-live="polite"
              ...
              <div
                className="relative h-3 w-full overflow-hidden rounded-full border border-black/10 bg-white/70 shadow-inner"
                role="progressbar"
                aria-valuemin={0}
                aria-valuemax={100}
              ...
              >
                {KIND_OPTIONS.map((o) => {
                  ...
                  return (
                    ...
                    active
                          ? "bg-sky-500/25 text-sky-950 shadow-[0_0_0_1px_rgba(56,189,248,0.35)]"
                          : "border border-black/10 bg-white/70 text-zinc-500 hover:border-black/20 hover:bg-white hover:text-[#171412]"
                      }`}
                    >
                      {o.label}
                    </button>
              ...
              <div className="flex flex-wrap gap-2">
                {presets.map((p) => (
                  <button
                    ...
                    className="focus-ring tap-target flex min-h-[44px] flex-col items-center justify-center rounded-xl border border-sky-500/20 bg-white/60 px-3 py-2 text-center transition-[transform,background-color,border-color] duration-200 hover:border-sky-500/40 hover:bg-white active:scale-[0.98] disabled:opacity-40"
                  >
            ...
            <div className="mb-1">
                    ...
                    {logs.map((l) => (
                      ...
                      >
                        <div className="min-w-0">
                          <p className="font-bold text-[#171412]">
              ...
              <p className="mt-4 border-t border-black/10 pt-4 text-[10px] leading-relaxed text-zinc-600">
